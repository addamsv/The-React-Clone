...
class Component extends ElementClass {
  ...
  setState(partialState: any) {
    ...
    updateInstance(this.render(), this.internalInstance.dom, this.internalInstance.element);
  }
}

const updateChild = (domElement: any, children: Array<any | string> | string, compareChildren?: any) => {
    ...
    if (element && typeof element === 'object') {
      ...
      if (typeof element.type === 'function') {
        ...
        if (a !== b) {
          ...
          const cmpElement = compareChildren.props.children[indx].type(compareChildren.props.children[indx].props);

          // console.log('element FUNCTION - - - - - -', newElement);
          // console.log('element FUNCTION 2 - - - - -', cmpElement);
          ...
          if (!newElement && !cmpElement) {
            ...
            domElement.insertBefore(newElementInstance.dom[0], domElement.childNodes[indx]);
            Object.defineProperty(newElementInstance.dom[0], 'aDataRootCompnnt', {
              enumerable: false,
              configurable: false,
              writable: true,
              value: { dom: newElementInstance.dom[0], element, elementRenderFunction: element.type, elementRenderFunctionArgs: element.props }
            });
            
          } else {
            /* Update/Remove Element */
            updateInstance(newElement, domElement.children[indx - shiftIndexOfRemovedElement - 1], cmpElement);
      ...
      /** - - - - - - D I V  C O M P O N E N T S - - - - - - */
        ...
        if (a !== b) {
        /* cmp component */
          const status = update(element, domElement.childNodes[indx - shiftIndexOfRemovedElement], compareChildren.props.children[indx]);
        }
      }
    } else {
      /** - - - - - - S T R I N G S  or number... - - - - - - */
      if (element.toString() && (typeof element === 'string' || typeof element === 'number')) {

        if (element !== compareChildren.props.children[indx - 1]) {
          domElement.insertBefore(document.createTextNode(element as string), domElement.childNodes[indx]);
...
const update = (element: any, domElement: any, cmpElement?: Element): string => {
...
const isEventListener = (attr: string): boolean => {
  return attr.startsWith('on') && (attr.toLowerCase() in window || attr === 'onTouchStart');
}

const isPropNameValid = (propName: string): boolean => {
  return ! ['className', 'selected', 'style', 'key', 'ref', 'children', 'internaldataargs', 'internaldatadom'].some((el) => el === propName);
}
...
const isEntriesValid = (key: string, value: any) => {
  return ['type', 'props', 'ref', 'key'].some((currKey: string) => currKey === key) && value;
}
  
const React = {
  Component,

  createElement: (type?: any, properties?: any, ...child: any): Element | null => {
    ...
    if (child.length) {
      props.children = (child.length === 1 && (typeof child[0] === 'string' || typeof child[0] === 'number'))
        ? child[0].toString()
        : child.filter((container: any) => {
          ...
          if (container && (typeof container !== 'string' && typeof container !== 'number')) {
            Object
              .entries(container)
  ...
  put: (
    rootReactElement: any,
    ...reactElements: Array<Element>
  ) => {
...
const ReactDOM = {
  ...
  */
const reconcile = (parentDom: HTMLElement | null, instance: any, element: any): any => {
  if (instance === null) {
    // Create instance
...
const instantiate = (element: any, topNode?: any): { dom: Array<any>, element: any } => {
  const { type, props } = element;
...
const setProps = (topNode: any, element: any, props: any, compareElement?: Element) => {
  ...
  if (props && topNode) {
        ...
        if (isEventListener(prop)) {
          ...
          if (compareElement) {
            if (compareElement.props[prop] !== prop) {
              topNode.removeEventListener(prop.toLowerCase().substring(2), compareElement.props[prop]);
            }
          }
          ...
          if (prop === 'internaldata') {
            Object.defineProperty(topNode, 'aDataRootCompnnt', {
              ...
              configurable: false,
              writable: true,
              value: { dom: topNode, element, elementRenderFunction: value, elementRenderFunctionArgs: element.props.internaldataargs }
            });
            value.internalInstance = { dom: topNode, element, elementRenderFunction: value, elementRenderFunctionArgs: element.props.internaldataargs }
    ...
    if (props.selected !== 'undefined') {
      ...
      if (compareElement && topNode.getAttribute('selected') !== null && props.selected === false) {
    ...
    if (props.style) {
      if (compareElement) {
        if (compareElement.props.style) {
          /* isStyles equal */
          if (JSON.stringify(compareElement.props.style) !== JSON.stringify(props.style)) {
    ...
    return (newState?: any) => {
      ...
      if (
        ...
        if (typeof newState === 'function') {
          root.aDataRootCompnntState = Object.assign({}, root.aDataRootCompnntState, newState(root.aDataRootCompnntState));
        } else {
          root.aDataRootCompnntState = Object.assign({}, root.aDataRootCompnntState, newState);
        }
      }

      if (typeof root.aDataRootCompnntState === 'string' || typeof root.aDataRootCompnntState === 'number' || Array.isArray(root.aDataRootCompnntState)) {
      ...
      if (prevState !== newState.toString()) {
        ...
        updateInstance(root.aDataRootCompnnt.elementRenderFunction(root.aDataRootCompnnt.elementRenderFunctionArgs), root.aDataRootCompnnt.dom, root.aDataRootCompnnt.element);
      }
    } 
  }
