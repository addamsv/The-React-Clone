...
const Scene = () => {
  ...
  const getCrName = (currentHid: string) => {
    return DataManager.getSlideFirstCr(currentHid) || Container.getFirstCrName();
  }
  ...
  ? sectionsForInsert
    .map((currentHid: string, indx) => {
      ...
      return <Section key={indx} hid={scene.hid} currentHid={currentHid}
          crName={getCrName(currentHid)} jsn={Data.getJsn()} />;
    })
  : [];
  ...
  return (
    <>
      ...
      .map(([cr, crJsn]: [string, any]) => {
        if (!isCrNameNotValid(Container.getCrNameWithoutNum(cr))) {
          ...
          switch (type) {
            ...
            case 'form':
              ...
              // Data.getData({ [cr]: crJsn }, cr, 'content', type))
              return (
            ...
            case 'video':
              return (
                <video controls loop autoPlay key={`h${hid}-${crName}-${cr}-${type}`} id={`h${hid}-${crName}-${cr}-${type}`} className={`change-point ${type}h${hid} h${hid}-${crName}-${cr}-${type} animate`}>
                    <source  type="video/mp4" src={ Data.getData({ [cr]: crJsn }, cr, 'src', type) }>
